...
case "$PLATFORM" in
    termux)
        ...
        if [ -d "$HOME/storage/downloads" ]; then
            INSTALL_DIR="$HOME/storage/downloads/lazyfarmers"
        else
            INSTALL_DIR="$HOME/lazyfarmers"
        fi
        ;;
    macos)
        if command -v xdg-user-dir >/dev/null 2>&1; then
            INSTALL_DIR="$(xdg-user-dir DOWNLOAD)/lazyfarmers"
        else
            INSTALL_DIR="$HOME/Downloads/lazyfarmers"
        fi
        ;;
    linux)
        if command -v xdg-user-dir >/dev/null 2>&1; then
        ...
        INSTALL_DIR="$BASE_DIR/lazyfarmers"
        ;;
esac
...
if [ -d "$INSTALL_DIR" ]; then
    if [ -d "$INSTALL_DIR/.git" ]; then
        ...
        fail "Installation directory exists but is not a Lazy Farmers repository"
    fi
else
    info "Downloading Lazy Farmers"
...
echo
ok "Lazy Farmers installed successfully"
