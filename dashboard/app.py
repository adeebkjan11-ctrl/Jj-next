...
def monitor_config_api():
    ...
    if 'token' in data:
        if not isinstance(data['token'], str):
            return jsonify({'success': False, 'error': 'Token must be text'}), 400
        if data['token'].strip():
            allowed['token'] = data['token'].strip()
    if 'enabled' in data:
    ...
    result, error = _bot_loop_call(monitor.configure(g.owner, allowed))
    if error:
        # A Discord 401 is a bot credential error, not a dashboard session expiry.
        return jsonify({'success': False, 'error': error}), 400
