            ...
            if now - self.last_inv_time > 15:
                ...
                if missing_types:
                    ...
                    if actually_missing:
                        ...
                        self.bot.log("SYS", "[Gems] No active gems detected! Triggering inventory check.")
        ...
        if "hunt is empowered by" in content:
            ...
            if missing_types:
                ...
                if actually_missing:
                    ...
                    self.bot.log("SYS", f"[Gems] Active gems detected, but missing: {', '.join(actually_missing)}. Checking inventory...")
                    state.checking_gems[self.bot.user_id] = {
                        "time": time.time(),
                        "types": actually_missing
                    }
        ...
        if ("'s inventory" in content or "'s gems" in content) and "**" in content:
             if state.checking_gems.get(self.bot.user_id):
                ...
                if to_use:
                    ...
                    self.bot.log("SUCCESS", f"[Gems] Equipped: {use_cmd}")
                    self.last_inv_time = time.time()
                else:
                    self.bot.log("WARN", f"[Gems] Inventory checked, but no matching gems found for: {missing_types}")
